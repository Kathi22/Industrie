...
CertificateValidationListener {
	...
	public ValidationResult onValidate(Cert certificate,
		...
		if (!passedChecks.contains(CertificateCheck.Validity)) {
			Date today = new Date();
			final boolean isYoung = certificate.getCertificate().getNotBefore()
					.compareTo(today) > 0;
					final boolean isOld = certificate.getCertificate().getNotAfter()
							.compareTo(today) < 0;
		...
		if (!passedChecks.contains(CertificateCheck.Uri)) {
			...
			println("    ApplicationURI in ApplicationDescription = "
